...
async def test_convert_page_measures_height_and_calls_pdf(tmp_path):
    ...
    # Only the first page: avoids a blank sub-pixel-spillover second page and
    # keeps the output single-page so the whitespace trim applies.
    assert kwargs["page_ranges"] == "1"
